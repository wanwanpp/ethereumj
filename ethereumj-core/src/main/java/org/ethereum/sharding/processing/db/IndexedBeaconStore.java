...
public class IndexedBeaconStore implements BeaconStore {
    ...
    public synchronized void save(Beacon block, BigInteger chainScore, boolean canonical) {
        ...
        putIndexItem(block.getSlot(), item);
    ...
    public synchronized void reorgTo(Beacon fork) {
        ...
        long canonicalHeadNum = getCanonicalHead().getSlot();
        byte[] forkHead = fork.getHash();
        long forkHeadNum = fork.getSlot();

        // scan for fork head
        for (long num = fork.getSlot() + 1; num <= getMaxNumber(); ++num) {
    ...
    private ChainItem getChainItemByHash(byte[] hash) {
        ...
        List<ChainItem> generation = getGenerationByNumber(block.getSlot());
        if (generation == null)
            return null;
